    ...
    <>
      <nav className={`fixed w-full z-50 px-6 pt-12 pb-8 md:pt-10 md:pb-12 transition-all duration-300 bg-gradient-to-b from-black from-30% via-black/80 via-60% to-transparent ${showNav ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        <div className="flex gap-3 md:gap-4 items-center absolute right-6">
          <button className="hidden md:block text-base font-medium text-gray-200 hover:text-white transition-colors relative group py-2">
          ...
          <div className="relative group hidden sm:block">
            ...
            <button className="relative bg-black/80 hover:bg-black/90 px-5 md:px-6 py-2 md:py-2.5 rounded-full text-base font-bold transition-all duration-300 border border-white/10 backdrop-blur-md text-white active:scale-95">
              Contratar
            </button>
          </div>
      ...
      >
        ...
        <button 
          className="mt-4 bg-gradient-to-r from-[#6F70DE] to-[#85EDAF] text-black px-8 py-3 rounded-full text-lg font-bold shadow-lg shadow-[#6F70DE]/20 active:scale-95 transition-transform"
          onClick={() => setIsMobileMenuOpen(false)}
        >
          Contratar Ahora
        </button>
